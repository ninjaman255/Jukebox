...
Net:on("post_selection", function(event)
  return async(function ()
    ...
    elseif numberfiedPostID <= #Songs then
      response = await(Async.question_player(event.player_id,("Do you wish to change the song to "..Songs[tonumber(event.post_id)].title.."?")))
      if (response == 1) then
...
Net:on("object_interaction", function(event)
    ...
    if object.class ~= "Jukebox" and object.type ~= "Jukebox" then
      return
    end 
